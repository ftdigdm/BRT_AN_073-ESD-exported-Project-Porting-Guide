...
extern uint8_t textbuffer[1000];
extern UART_HandleTypeDef huart2;
#define eve_printf_debug APP_DBG
#define myprintf(M, ...)                                    \
{            \
	memset(textbuffer, 0, 1000); \
	snprintf(textbuffer,1000, M, ##__VA_ARGS__);            \
	HAL_UART_Transmit(&huart2,textbuffer,1000,10);          \
}
/** macro: PRINTF_LEVEL
 *    range: 0-3
 *    0: Disable printf
...
#define APP_PRINTF_ERROR(M, ...)            printf("[ERROR] %s:%s:%d: " M "\r\n",__FILE__, __func__, __LINE__, ##__VA_ARGS__)
...
#define EVE_HOST EVE_HOST_STM32L476GDISCOVERY
#endif
