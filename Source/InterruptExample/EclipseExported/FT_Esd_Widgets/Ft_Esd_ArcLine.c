...
void Ft_Esd_ArcLine_Render_Func(Ft_Esd_ArcLine *context, int x, int y, int radius, int border, ft_argb32_t color, ft_bool_t isClockwise, int origin, int value)
{
...
#endif
	EVE_CoCmd_dl(phost, SAVE_CONTEXT()); // A
	...
	if (value < 360 && value > 0)
	{
		if (!context->ShowStartPoint)
		{
			...
			EVE_CoCmd_dl(phost, VERTEX2F((int)x0, (int)y0));
			EVE_CoCmd_dl(phost, VERTEX2F((int)x1, (int)y1));
	...
	EVE_CoCmd_dl(phost, RESTORE_CONTEXT()); // A

	EVE_CoCmd_dl(phost, SAVE_CONTEXT()); // B
	...
	EVE_CoCmd_dl(phost, RESTORE_CONTEXT()); // B

	if (endAngle > 360 && (endAngle % 90) > 0)
	{
		EVE_CoCmd_dl(phost, SAVE_CONTEXT()); // C
		...
		EVE_CoCmd_dl(phost, RESTORE_CONTEXT()); // C
		...
		if (value < 360 && value > 0)
		{
			if (!context->ShowEndPoint)
			{
				...
				EVE_CoCmd_dl(phost, SAVE_CONTEXT()); // D
				...
				EVE_CoCmd_dl(phost, RESTORE_CONTEXT()); // D
			}
		}
	}
